...
class Seq2Seq2(BaseModel):
    ...
    def serve_batch(self):
        ...
        while True:
            ...
            if from_idx+self.params['batch_size'] > len(self.input_texts):
                from_idx = 0
            print(self.encoder_input_data.shape, self.decoder_input_data.shape, self.decoder_target_data.shape)
    ...
    def split_count_data(self):
        ...
        input_texts = tokenizer.texts_to_sequences(self.input_texts)
        target_texts = tokenizer.texts_to_sequences(self.target_texts)

        self.input_texts = pad_sequences(input_texts, maxlen=self.params['max_seq_length'], padding='post')
        self.target_texts = pad_sequences(target_texts, maxlen=self.params['max_seq_length'], padding='post')
